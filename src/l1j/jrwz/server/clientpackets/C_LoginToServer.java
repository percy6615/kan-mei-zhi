...
import l1j.jrwz.server.templates.L1GetBackRestart;
...
public class C_LoginToServer extends ClientBasePacket {
    ...
    public C_LoginToServer(byte abyte0[], ClientThread client)
        ...
        if (client.getActiveChar() != null) {
            _log.info("同一个角色重复登陆，强制切断 " + client.getHostname() + ") 的连接。");
            client.close();
            return;
        }
        ...
        if (Config.LEVEL_DOWN_RANGE != 0) {
            if (pc.getHighLevel() - pc.getLevel() >= Config.LEVEL_DOWN_RANGE) {
                _log.info("登陆请求超出了容忍的 等级下降的角色: char=" + charName + " account="
        ...
        _log.info("角色进入游戏: char=" + charName + " account=" + login + " host="
                + client.getHostname());
