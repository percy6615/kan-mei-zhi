...
public class NpcSpawnTable {
    ...
    private final Map<Integer, L1Spawn> _spawntable = new HashMap<Integer, L1Spawn>();
    ...
    private void fillNpcSpawnTable() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (!Config.ALT_TALKINGSCROLLQUEST) { // 说话卷轴任务
                    int npcid = rs.getInt("id");
                    if (npcid >= 544 && npcid <= 558) {
                        continue;
                    }
                }
        ...
        _log.config("加载NPC配置资料表 " + _spawntable.size() + " 个");
        _log.fine("NPC总数 " + spawnCount + " 个");
    }
