...
import l1j.jrwz.server.serverpackets.S_DeleteInventoryItem;
import l1j.jrwz.server.serverpackets.S_ItemAmount;
import l1j.jrwz.server.serverpackets.S_ItemColor;
import l1j.jrwz.server.serverpackets.S_ItemName;
import l1j.jrwz.server.serverpackets.S_ItemStatus;
...
public class L1PcInventory extends L1Inventory {
    ...
    public L1ItemInstance[] getRingEquipped() {
        ...
        for (Object itemObject : _items) {
            L1ItemInstance item = (L1ItemInstance) itemObject;
            if (item.getItem().getType2() == L1Item.TYPE2_ARMOR && item.getItem().getType() == 9 && item.isEquipped()) {
    ...
    public void setEquipped(L1ItemInstance item, boolean equipped, boolean loaded, boolean changeWeapon) {
        if (item.isEquipped() != equipped) { // 設定値と違う場合だけ処理
            ...
            if (!loaded) { // 最初の読込時はＤＢパケット関連の処理はしない
                ...
                if (temp.getType2() == L1Item.TYPE2_WEAPON && changeWeapon == false) { // 武器の場合はビジュアル更新。ただし、武器の持ち替えで武器を脱着する時は更新しない
