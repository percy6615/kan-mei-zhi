...
public class C_Attr extends ClientBasePacket {
    ...
    private static void renamePet(L1PetInstance pet, String name) {
        ...
        if (PetTable.isNameExists(name)) {
            pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$327)); // 同樣的名稱已經存在。
            return;
        }
        L1Npc l1npc = NpcTable.getInstance().getTemplate(pet.getNpcId());
        if (!(pet.getName().equalsIgnoreCase(l1npc.get_name()))) {
            pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$326)); // 一旦你已決定就不能再變更。
            return;
        }
        pet.setName(name);
    ...
    public C_Attr(byte abyte0[], ClientThread clientthread) throws Exception {
        ...
        case 97: // \f3%0%s 想加入你的血盟。你接受嗎。(Y/N)
            ...
            if (joinPc != null) {
                if (c == 0) { // No
                    joinPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$96, pc.getName())); // \f1%0%s
                                                                                                  // 拒絕你的請求。
                } else if (c == 1) { // Yes
                    ...
                    if (clan != null) {
                        ...
                        if (joinPc.getClanid() == 0) { // 加入玩家未加入血盟
                            ...
                            if (maxMember <= clanMembersName.length) { // 血盟還有空間可以讓玩家加入
                                ...
                                new S_ServerMessage(L1SystemMessageId.$188, pc.getName()));
                                return;
                            }
                            for (L1PcInstance clanMembers : clan.getOnlineClanMember()) {
                                clanMembers.sendPackets(new S_ServerMessage(L1SystemMessageId.$94, joinPc.getName())); // \f1你接受%0當你的血盟成員。
                            }
                            ...
                            joinPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$95, clanName)); // \f1加入%0血盟。
                        } else { // 如果是王族加入（聯合血盟）
                            if (Config.CLAN_ALLIANCE) {
                                changeClan(clientthread, pc, joinPc, maxMember);
                            } else {
                                joinPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$89)); // \f1你已經有血盟了。
                            }
                        }
                    }
                }
        ...
        case 222: // %0 血盟要結束戰爭。是否接受？(Y/N)
            ...
            if (c == 0) { // No
                if (i == 217) {
                    enemyLeader.sendPackets(new S_ServerMessage(L1SystemMessageId.$236, clanName)); // %0
                                                                                                    // 血盟拒絕你的宣戰。
                } else if (i == 221 || i == 222) {
                    enemyLeader.sendPackets(new S_ServerMessage(L1SystemMessageId.$237, clanName)); // %0
                                                                                                    // 血盟拒絕你的提案。
                }
            } else if (c == 1) { // Yes
                if (i == 217) {
        ...
        case 252: // \f2%0%s 要與你交易。願不願交易？ (Y/N)
            ...
            if (trading_partner != null) {
                if (c == 0) // No
                {
                    trading_partner.sendPackets(new S_ServerMessage(L1SystemMessageId.$253, pc.getName())); // %0%d
                                                                                                            // 拒絕與你交易。
                    pc.setTradeID(0);
        ...
        case 512: // 請輸入血盟小屋名稱?
            ...
            if (name.length() <= 16) {
                ...
                pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$513)); // 血盟小屋名稱太長。
            }
            break;
        ...
        case 630: // %0%s 要與你決鬥。你是否同意？(Y/N)
            ...
            if (c == 0) {
                ...
                fightPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$631, pc.getName())); // %0%d
                                                                                                // 拒绝了与你的决斗。
            } else if (c == 1) {
        ...
        case 653: // 若你離婚，你的結婚戒指將會消失。你決定要離婚嗎？(Y/N)
            ...
            if (c == 0) { // No
                ...
                if (target653 != null) {
                    ...
                    target653.save();
                    target653.sendPackets(new S_ServerMessage(L1SystemMessageId.$662)); // \f1你(妳)目前未婚。
                } else {
            ...
            pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$662)); // \f1你(妳)目前未婚。
            break;
        ...
        case 654: // %0 向你(妳)求婚，你(妳)答應嗎?
            ...
            if (partner != null) {
                if (c == 0) { // No
                    partner.sendPackets(new S_ServerMessage(L1SystemMessageId.$656, pc.getName())); // %0
                                                                                                    // 拒絕你(妳)的求婚。
                } else if (c == 1) { // Yes
                    pc.setPartnerId(partner.getId());
                    pc.save();
                    pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$790)); // 倆人的結婚在所有人的祝福下完成
                    pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$655, partner.getName())); // 恭喜!!
                                                                                                    // %0
                                                                                                    // 已接受你(妳)的求婚。

                    partner.setPartnerId(pc.getId());
                    partner.save();
                    partner.sendPackets(new S_ServerMessage(L1SystemMessageId.$790)); // 恭喜!!
                                                                                      // %0
                                                                                      // 已接受你(妳)的求婚。
                    partner.sendPackets(new S_ServerMessage(L1SystemMessageId.$655, pc.getName())); // 恭喜!!
                                                                                                    // %0
                                                                                                    // 已接受你(妳)的求婚。
                }
            }
        ...
        case 738: // 恢復經驗值需消耗%0金幣。想要恢復經驗值嗎?
            ...
            if (c == 0) { // No
                ...
                if (pc.getInventory().consumeItem(L1ItemId.ADENA, cost)) {
                    ...
                    pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$189)); // \f1金幣不足。
                }
            }
            break;
        ...
        case 951: // 您要接受玩家 %0%s 提出的隊伍對話邀請嗎？(Y/N)
            ...
            if (chatPc != null) {
                if (c == 0) { // No
                    chatPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$423, pc.getName())); // %0%s
                                                                                                   // 拒絕了您的邀請。
                    pc.setPartyID(0);
                } else if (c == 1) { // Yes
                    if (chatPc.isInChatParty()) {
                        if (chatPc.getChatParty().isVacancy() || chatPc.isGm()) {
                            ...
                            chatPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$417)); // 你的隊伍已經滿了，無法再接受隊員。
                        }
                    } else {
                        L1ChatParty chatParty = new L1ChatParty();
                        chatParty.addMember(chatPc);
                        chatParty.addMember(pc);
                        chatPc.sendPackets(new S_ServerMessage(L1SystemMessageId.$424, pc.getName())); // %0%s
                                                                                                       // 加入了您的隊伍。
                    }
                }
            }
        ...
        case 953: // 玩家 %0%s 邀請您加入隊伍？(Y/N)
            ...
            if (target != null) {
                if (c == 0) { // No
                    target.sendPackets(new S_ServerMessage(L1SystemMessageId.$423, pc.getName())); // %0%s
                                                                                                   // 拒絕了您的邀請。
                    pc.setPartyID(0);
                } else if (c == 1) { // Yes
                    if (target.isInParty()) {
                        ...
                        if (target.getParty().isVacancy() || target.isGm()) {
                            ...
                            // 組隊滿了
                            target.sendPackets(new S_ServerMessage(L1SystemMessageId.$417)); // 你的隊伍已經滿了，無法再接受隊員。
                        }
                    } else {
                        // 還沒有組隊，建立一個新組隊
                        ...
                        party.addMember(pc);
                        target.sendPackets(new S_ServerMessage(L1SystemMessageId.$424, pc.getName())); // %0%s
                                                                                                       // 加入了您的隊伍。
                    }
                }
            }
            ...
            if (readC() == 1) {
                ...
                if (s.toLowerCase().equals("str".toLowerCase())) {
                    ...
                    if (pc.getBaseStr() < 35) {
                        ...
                        pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$481)); // \f1屬性最大值只能到35。
                                                                                     // 請重試一次。
                    }
    ...
    private void callClan(L1PcInstance pc) {
        ...
        if (locX == 0 && locY == 0 || !map.isPassable(locX, locY) || isExsistCharacter) {
            ...
            pc.sendPackets(new S_ServerMessage(L1SystemMessageId.$627));
            return;
        }
    ...
    private void changeClan(ClientThread clientthread, L1PcInstance pc, L1PcInstance joinPc, int maxMember) {
        ...
        if (clan != null && oldClan != null && joinPc.isCrown() && // 自己的王族
            ...
            for (L1PcInstance element : clanMember) {
                element.sendPackets(new S_ServerMessage(L1SystemMessageId.$94, joinPc.getName())); // \f1你接受%0當你的血盟成員。
            }
            ...
            for (String element : oldClanMemberName) {
                ...
                if (oldClanMember != null) { // 舊血盟成員在線上
                    ...
                    oldClanMember.sendPackets(new S_ServerMessage(L1SystemMessageId.$95, clanName)); // \f1加入%0血盟。
                } else { // 舊血盟成員不在線上
                    try {
